...
pub fn solve(input: &Box<[Vec<ElementKind>]>) -> Option<i32> {
    let mut iter = input.iter();
    if let Some(val) = iter.next() {
        let mut result = val.to_vec();
        for next_val in iter {
            result = add_snailfish(result, next_val.to_vec());
...
pub fn magnitude(mut snailfish_magnitude: Vec<ElementKind>) -> Option<i32> {
